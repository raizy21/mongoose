...
export const addBookToReadingList = asyncHandler(async (req, res) => {
  ...
  updatedUser = await User.findOneAndUpdate(
    { _id: req.params.id, "readingList.bookRefId": { $ne: bookRefId } }, // $ne (not equal) ensures no duplicate bookRefId
    { $push: { readingList: { bookRefId, status } } },
    { new: true }
  );
...
export const updateBookStatusInReadingList = asyncHandler(async (req, res) => {
  ...
  const bookEntry = user.readingList.id(req.params.bookId); // bookId here is the _id of the book entry in the readingList array (should be on the same level as the status property)
